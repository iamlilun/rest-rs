...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        CREATE TABLE IF NOT EXISTS `users` (
            ...
            `password` varchar(30) NOT NULL DEFAULT '' COMMENT '密碼',
            `token` varchar(255) NOT NULL DEFAULT '' COMMENT 'token',
            `name` varchar(30) NOT NULL DEFAULT '' COMMENT '暱稱',
            `role` tinyint(1) NOT NULL DEFAULT 1 COMMENT '權限 1 => user, 99 => admin',
            `api_key` varchar(100) NOT NULL DEFAULT 1 COMMENT 'bybit api key',
            `secret_key` varchar(100) NOT NULL DEFAULT 1 COMMENT 'bybit secret key',
